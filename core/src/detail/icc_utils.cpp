...
std::vector<uint8_t> ExtractIccFromJpegBytes(const std::vector<uint8_t>& buf) {
    ...
    for (auto& c : chunks)
        profile.insert(profile.end(), c.data, c.data + c.size);
    return profile;
}
...
cv::Mat ReadJpegCmykFromMem(const uint8_t* data, size_t size) {
    ...
    bool is_cmyk = (cinfo.jpeg_color_space == JCS_CMYK ||
...
cv::Mat ConvertCmykToSrgb(const cv::Mat& cmyk, const std::vector<uint8_t>& icc) {
    ProfilePtr src_profile(cmsOpenProfileFromMem(icc.data(), static_cast<cmsUInt32Number>(icc.size())));
    ...
    TransformPtr xform(cmsCreateTransform(
        src_profile.get(), TYPE_CMYK_8, dst_profile.get(), TYPE_BGR_8,
        INTENT_PERCEPTUAL, 0));
    if (!xform) return {};
...
cv::Mat ConvertRgbProfileToSrgb(const cv::Mat& img, const std::vector<uint8_t>& icc) {
    ...
    TransformPtr xform(cmsCreateTransform(
        src_profile.get(), TYPE_BGR_8, dst_profile.get(), TYPE_BGR_8,
        INTENT_PERCEPTUAL, 0));
    if (!xform) return {};
...
cv::Mat LoadImageIcc(const uint8_t* data, size_t size) {
    ...
    if (is_jpeg) {
        icc = ExtractIccFromJpegBytes(buf_vec);
    }
