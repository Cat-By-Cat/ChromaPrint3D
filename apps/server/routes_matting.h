...
inline void RegisterMattingRoutes(ServerContext& ctx) {
    ...
    ctx.server.Post(
        ...
        [&ctx](const httplib::Request& req, httplib::Response& res) {
            ...
            if (req.form.has_field("method")) {
                method = req.form.get_field("method");
            }
    ...
    ctx.server.Get("/api/matting/tasks/:id/mask",
                   [&ctx](const httplib::Request& req, httplib::Response& res) {
                       AddCorsHeaders(req, res);
                       std::string id = req.path_params.at("id");
                       if (!ctx.matting_task_mgr.WithTask(id, [&](const MattingTaskInfo& task) {
                               ...
                               SetBinaryResponse(res, task.result.mask_png, "image/png",
                                                 "mask.png");
                           })) {
                           SetJsonResponse(res, ErrorJson("Matting task not found"), 404);
                       }
    ...
    ctx.server.Get("/api/matting/tasks/:id/foreground",
                   [&ctx](const httplib::Request& req, httplib::Response& res) {
                       AddCorsHeaders(req, res);
                       std::string id = req.path_params.at("id");
                       if (!ctx.matting_task_mgr.WithTask(id, [&](const MattingTaskInfo& task) {
                               ...
                               SetBinaryResponse(res, task.result.foreground_png, "image/png",
                                                 "foreground.png");
                           })) {
                           SetJsonResponse(res, ErrorJson("Matting task not found"), 404);
                       }
    ...
    ctx.server.Delete("/api/matting/tasks/:id",
                      [&ctx](const httplib::Request& req, httplib::Response& res) {
                          AddCorsHeaders(req, res);
                          std::string id = req.path_params.at("id");
                          if (ctx.matting_task_mgr.DeleteTask(id)) {
                              ...
                              SetJsonResponse(res, ErrorJson("Task not found or still running"),
                                              404);
                          }
                      });
}
