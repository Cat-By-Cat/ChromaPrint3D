...
DataType CvDepthToDataType(int depth) {
    switch (depth) {
        case CV_8U:  return DataType::kUInt8;
        case CV_8S:  return DataType::kInt8;
        case CV_32S: return DataType::kInt32;
        case CV_32F: return DataType::kFloat32;
        default:
...
int DataTypeToCvDepth(DataType dtype) {
    switch (dtype) {
        case DataType::kUInt8:   return CV_8U;
        case DataType::kInt8:    return CV_8S;
        case DataType::kInt32:   return CV_32S;
        case DataType::kFloat32: return CV_32F;
        default:
            throw InputError("DataType " + DataTypeName(dtype) +
                             " has no cv::Mat equivalent");
    }
}
...
Tensor Tensor::Wrap(DataType dtype, std::vector<int64_t> shape,
...
Tensor Tensor::FromCvMat(const cv::Mat& mat) {
    ...
    if (channels == 1) {
        shape = {static_cast<int64_t>(mat.rows),
                 static_cast<int64_t>(mat.cols)};
    } else {
        shape = {static_cast<int64_t>(mat.rows),
                 static_cast<int64_t>(mat.cols),
...
int64_t Tensor::Shape(int axis) const {
    if (axis < 0) axis += NDim();
    if (axis < 0 || axis >= NDim()) {
        throw InputError("Tensor::Shape axis out of range");
    }
...
cv::Mat Tensor::ToCvMat(bool chw_to_hwc) const {
    ...
    if (chw_to_hwc && ndim >= 3) {
        ...
        int64_t c{}, h{}, w{};
        if (ndim == 4) {
            c = shape_[1]; h = shape_[2]; w = shape_[3];
        } else if (ndim == 3) {
            c = shape_[0]; h = shape_[1]; w = shape_[2];
        } else {
        ...
        const size_t plane_bytes = static_cast<size_t>(hi) *
                                   static_cast<size_t>(wi) * elem_size;
        ...
        for (int ch = 0; ch < ci; ++ch) {
            ...
            planes.emplace_back(hi, wi, CV_MAKETYPE(depth, 1),
                                const_cast<void*>(plane_ptr));
        }
    ...
    if (ndim == 2) {
        return cv::Mat(static_cast<int>(shape_[0]),
                       static_cast<int>(shape_[1]),
                       CV_MAKETYPE(depth, 1),
                       const_cast<void*>(data_)).clone();
    }
    if (ndim == 3) {
        return cv::Mat(static_cast<int>(shape_[0]),
                       static_cast<int>(shape_[1]),
                       CV_MAKETYPE(depth, static_cast<int>(shape_[2])),
                       const_cast<void*>(data_)).clone();
    }
