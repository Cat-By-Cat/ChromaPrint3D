...
inline void RegisterSessionRoutes(ServerContext& ctx) {
    ...
    ctx.server.Delete(
        "/api/session/colordbs/:name", [&ctx](const httplib::Request& req, httplib::Response& res) {
            AddCorsHeaders(req, res);
            std::string token = GetSessionToken(req);
            if (token.empty()) {
            ...
            spdlog::info("Session ColorDB deleted: name={}, session={}", name, token.substr(0, 8));
            SetJsonResponse(res, json{{"deleted", true}});
        });

    // Download session ColorDB as JSON
    ctx.server.Get("/api/session/colordbs/:name/download", [&ctx](const httplib::Request& req,
    ...
    ctx.server.Post("/api/session/colordbs/upload", [&ctx](const httplib::Request& req,
                                                           httplib::Response& res) {
        AddCorsHeaders(req, res);
        ...
        spdlog::info("ColorDB uploaded: name={}, session={}", final_name, token.substr(0, 8));
