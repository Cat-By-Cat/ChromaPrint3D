...
inline void RegisterCalibrationRoutes(ServerContext& ctx) {
    ...
    ctx.server.Post("/api/calibration/generate-board", [&ctx](const httplib::Request& req,
        ...
        try {
            ...
            if (cached) {
                ...
                spdlog::info("Geometry cache miss for {}ch/{}L, generating...", num_ch, c_layers);
            ...
            SetJsonResponse(res, ErrorJson(std::string("Generation failed: ") + e.what()), 500);
        }
    });

    // Download calibration board 3MF
    ctx.server.Get("/api/calibration/boards/:id/3mf", [&ctx](const httplib::Request& req,
                                                             httplib::Response& res) {
        AddCorsHeaders(req, res);
        ...
        std::string filename = entry->meta.name.empty() ? "calibration_board" : entry->meta.name;
        filename += ".3mf";
        SetBinaryResponse(res, entry->model_3mf,
                          "application/vnd.ms-package.3dmanufacturing-3dmodel+xml", filename);
    });

    // Download calibration board meta JSON
    ctx.server.Get("/api/calibration/boards/:id/meta", [&ctx](const httplib::Request& req,
                                                              httplib::Response& res) {
        AddCorsHeaders(req, res);
        std::string id    = req.path_params.at("id");
        const auto* entry = ctx.board_cache.Find(id);
        ...
        std::string filename =
            entry->meta.name.empty() ? "calibration_board_meta" : entry->meta.name + "_meta";
        filename += ".json";
    ...
    ctx.server.Post("/api/calibration/generate-8color-board", [&ctx](const httplib::Request& req,
                                                                     httplib::Response& res) {
        AddCorsHeaders(req, res);
        ctx.session_mgr.CleanExpired();
        ...
        if (num_ch != ctx.recipe_store.num_channels) {
            SetJsonResponse(
                res,
                ErrorJson("palette size must be " + std::to_string(ctx.recipe_store.num_channels)),
                400);
            return;
        }
        ...
        if (!board_set) {
            SetJsonResponse(res, ErrorJson("Invalid board_index: " + std::to_string(board_index)),
                            400);
            return;
        }
        ...
        cfg.recipe.layer_order   = (ctx.recipe_store.layer_order == "Bottom2Top")
        ...
        try {
            ...
            CalibrationBoardResult result;
            const auto* cached = ctx.geometry_cache.Find(num_ch, c_layers, board_index);
            if (cached) {
                spdlog::info("Geometry cache hit for 8ch board {}", board_index);
                result = BuildResultFromMeshes(*cached, cfg.palette);
            } else {
                spdlog::info("Generating 8ch board {} ({}x{}, {} recipes)...", board_index,
                             board_set->grid_rows, board_set->grid_cols, board_set->recipes.size());
                ...
                CalibrationBoardMeshes meshes = GenCalibrationBoardMeshesFromMeta(std::move(meta));
                result                        = BuildResultFromMeshes(meshes, cfg.palette);
                ctx.geometry_cache.Store(num_ch, c_layers, std::move(meshes), board_index);
            }

            std::string meta_str = result.meta.ToJsonString();
            std::string board_id = ctx.board_cache.Store(std::move(result));
            spdlog::info("8-color board {} generated: id={}", board_index, board_id.substr(0, 8));
            json resp;
            resp["board_id"] = board_id;
            resp["meta"]     = json::parse(meta_str);
            SetJsonResponse(res, resp);
    ...
    ctx.server.Post("/api/calibration/build-colordb", [&ctx](const httplib::Request& req,
                                                             httplib::Response& res) {
        AddCorsHeaders(req, res);
        ...
        try {
            ...
            SetJsonResponse(res, ErrorJson(std::string("Invalid meta JSON: ") + e.what()), 400);
            return;
        }
        ...
        try {
            ...
            spdlog::info("ColorDB built: name={}, session={}", db_name, token.substr(0, 8));
            SetJsonResponse(res, db_json);
        } catch (const std::exception& e) {
            spdlog::error("ColorDB build failed: {}", e.what());
            SetJsonResponse(res, ErrorJson(std::string("ColorDB build failed: ") + e.what()), 500);
        }
    });
}
