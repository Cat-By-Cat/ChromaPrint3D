...
DataType OrtTypeToDataType(ONNXTensorElementDataType ort_type) {
    switch (ort_type) {
        ...
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_BOOL:     return DataType::kBool;
        default:
...
OrtInferSession::OrtInferSession(std::unique_ptr<Ort::Session> session,
                                 Device device,
                                 Ort::Env& /*env*/)
...
void OrtInferSession::PopulateModelInfo() {
    ...
    spdlog::debug("OrtSession loaded: {} input(s), {} output(s)",
...
ONNXTensorElementDataType DataTypeToOrtType(DataType dtype) {
    switch (dtype) {
        ...
        case DataType::kBool:    return ONNX_TENSOR_ELEMENT_DATA_TYPE_BOOL;
    }
...
std::vector<Ort::Value> OrtInferSession::RunImpl(
...
std::unordered_map<std::string, Tensor> OrtInferSession::Run(
    ...
    for (const auto& name : input_names_) {
        ...
        if (it == inputs.end()) {
            throw InputError("Missing input tensor: '" + name + "'");
        }
...
std::vector<Tensor> OrtInferSession::Run(const std::vector<Tensor>& inputs) {
    if (inputs.size() != input_names_.size()) {
        throw InputError("Expected " + std::to_string(input_names_.size()) +
    ...
    for (auto& val : ort_outputs) {
        result.push_back(OrtValueToTensor(val));
    }
    return result;
}
