...
支持在转换时显式配置底板层数（`base_layers`）与双面生成（`double_sided`）。
...
You can also explicitly control base layers (`base_layers`) and double-sided generation (`double_sided`) during conversion.
