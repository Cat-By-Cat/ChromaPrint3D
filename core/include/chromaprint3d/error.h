...
public:
    explicit FormatError(const std::string& msg) : Error(ErrorCode::FormatError, msg) {}
};

/// Incompatible configuration across modules.
class CHROMAPRINT3D_API ConfigError : public Error {
public:
    explicit ConfigError(const std::string& msg) : Error(ErrorCode::ConfigMismatch, msg) {}
};

/// Color matching found no usable candidate.
class CHROMAPRINT3D_API MatchError : public Error {
public:
    explicit MatchError(const std::string& msg) : Error(ErrorCode::NoValidCandidate, msg) {}
};
