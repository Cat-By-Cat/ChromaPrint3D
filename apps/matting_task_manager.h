...
inline std::string SubmitMatting(MattingTaskMgr& mgr, const MattingRegistry& registry,
                                 std::vector<uint8_t> image_buffer, const std::string& method,
                                 const std::string& image_name) {
    MattingTaskInfo initial;
    initial.method     = method;
    initial.image_name = image_name;

    return mgr.Submit(std::move(initial), [&registry, buf = std::move(image_buffer),
                                           method](const std::string& id, auto update) mutable {
